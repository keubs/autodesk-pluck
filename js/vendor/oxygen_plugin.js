...
var OXYGEN_OPTIONS = {
  ...
  provider: 'autodesk',
  providers_endpoint: {
    autodesk: 'https://accounts-staging.autodesk.com/SignIn'
  },
  openid_response: 'oxygen_openid_response.html',
  refresh_response: 'oxygen_refresh_response.html',
  required_array: ['namePerson', 'autodesk/userid', 'namePerson/first', 'namePerson/last', 'contact/email']
};
...
var mergeOptions = function(options, default_options) {
  for (var key in default_options) {
    if (default_options.hasOwnProperty(key) && options.hasOwnProperty(key)) {
      if (typeof options[key] === 'object' && typeof default_options[key] === 'object') {
        options[key] = mergeOptions(options[key], default_options[key]);
      } else if (!options.hasOwnProperty(key)) {
        options[key] = default_options[key];
      }
    } else if (default_options.hasOwnProperty(key) && !options.hasOwnProperty(key)) {
      options[key] = default_options[key];
    }
  }
...
var OxygenOverlay = function(options) {
  ...
  var fn = {
    setStyles: function() {
      ...
      jQuery(close_btn).css({
        ...
        backgroundImage: options.close_button.image,
        cursor: 'pointer'
      });
    },
  ...
  this.open = function(new_content, size) {
    ...
    if (!document.getElementById('oxygen-overlay')) {
      ...
      if (!eventsAttached) {
        jQuery(close_btn).bind('click', function(e) { self.close.call(self); });
    ...
    jQuery(window).bind('resize.oxygen', function(e) { fn.resizeTo.call(self, options); });
    jQuery(document).bind('keydown.oxygen', function(e) { fn.keydown.call(self, e); });
    if (options.hideOnOverlayClick) jQuery(overlay).bind('click.oxygen', function(e) { self.close.call(self); });
...
var Oxygen = this.Oxygen = (function() {
  var adapter,
      ...
      server_root = window.location.href.split('/').slice(0, -1).join('/') + '/',
      fn = {
        init: function(opts) {
          if (adapter) return;
          opts = opts || {};
        ...
        getBaseOpenIDProviderURL: function(provider, immediate) {
          var providerURL = options.providers_endpoint[provider] +
                            ...
                            "&openid.ax.required=" + encodeURIComponent(options.required_array) +
                            "&openid.ax.mode=fetch_request";

          OXYGEN_OPTIONS.required_array.forEach(function(value){
              providerURL += "&openid.ax.type."+value+"=" + encodeURIComponent("http://axschema.org/"+value);
              providerURL += "&openid.ax.count."+value+"=1";
          });
  ...
  return {
    ...
    show: function(href, viewmode, size) {
      ...
      adapter.open(href || fn.getBaseOpenIDProviderURL(options.provider, false), size);
    },
    hide: function() {
    ...
    checkImmediate: function(url) {
      ...
      //var iframe = OxygenIframe(fn.getBaseOpenIDProviderURL(options.provider, true));
